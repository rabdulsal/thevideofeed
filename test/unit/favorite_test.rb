...
class FavoriteTest < ActiveSupport::TestCase
  ...
  test "requires video and person" do
    ...
    assert Favorite.new(key: 'key', title: 'title').invalid?
  ...
  test "doesn't allow two favorites for the same video by the same user" do
    @person.favorites.create!(key: 'key', title: 'title')
  ...
  test "creates a new video if no one else favorited it, uses an existing video if found" do
    ...
    assert_no_difference('Video.count') do
      assert_difference('Favorite.count') do
        @person2.favorites.create!(key: 'key', title: 'title')
      end
    end
  end
  ...
  test "sets first person to the older favorite" do
    @person.favorites.create!(key: 'key', title: 'title', created_at: Date.yesterday.to_time)
    @person2.favorites.create!(key: 'key', title: 'title', created_at: Date.today.to_time)
